(function (window, undefined) {
    ...
    function initDatePicker(element = false){
    ...
    function focusOnLastProjekt(){
        let lastProjekt = $("#projekts-div .card:last-child select:first");
    ...
    $("#ajaxForm").on("submit", function (e) {
        ...
        $.ajax({
            ...
            error: function (e) {
                let resp = $.parseJSON(e.responseText);
                if( e.status === 422 ) {
    ...
    $("#addProject").click(function () {
        let project = $($("#newProjekt").html());
        let currentTimeStamp = + new Date();
        project.find("input, select, textarea").each(function () {
            let name = $(this).attr("name")  + "[" + currentTimeStamp + "]";
