...
@section('title', __('locale.Create Reference'))
...
@section('content')
    <section xmlns="http://www.w3.org/1999/html">
        ...
        <form class="form form-unload-check" id="ajaxForm" action="{{ route('references.store') }}" enctype="multipart/form-data" method="post">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">@lang('locale.Create Reference')</h4>
                </div>
                <div class="card-body">
                    @csrf
                    <div class="row">
                                    ...
                                    <div class="mb-1 ">
                                        ...
                                        @foreach($objektCategories as $code =>  $name)
                                            <div class="form-check form-check-inline">
                                                <input
                                                    {{ $loop->first && empty(old('category')) ? "checked" : "" }}
                                                    {{ old("category") == $code ? "checked" : "" }}
                                                    type="radio"
                                                    id="{{$name}}-cat-column"
                                ...
                                <div class="col-lg-6 col-md-6 col-12">
                                    <div class="mb-1 ">
                                        ...
                                        @foreach($objektTypeCodes as $code =>  $name)
                                            <div class="form-check form-check-inline">
                                                <input
                                                    {{ $loop->first && empty(old('objekt_type_code')) ? "checked" : "" }}
                                                    {{ old("objekt_type_code") == $code ? "checked" : "" }}
                                                    type="radio"
                                                    id="{{$name}}-column"
                                                    class="form-check-input"
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-2 pt-lg-2 text-lg-start text-end">
                            <button type="submit" class="btn btn-primary mt-lg-5 me-1">@lang('locale.Save')</button>
                        </div>
                    </div>
                </div>
            </div>

            <div class="d-flex align-items-center flex-wrap justify-content-between border-bottom-secondary py-2 mb-2">
                <h4>@lang('locale.Projekts') </h4>
                <div class="d-flex align-items-center align-content-center"><a href="#" id="addProject" class="text-black d-flex align-items-center">@lang('locale.Add Projekt') &nbsp;<i class="fa fa-2x fa-plus-circle"></i></a></div>
            </div>
            <div id="projekts-div"></div>
        </form>
